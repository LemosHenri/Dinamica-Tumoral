...
class Database:
    ...
    def generate_trunk(self, time: float):
        ...
        self.trunk = np.arange(0, time, 1.0, dtype = np.float32).reshape(-1, 1)
    ...
    def trainning(self, size: int, file_name:str, checkpoints: int, batch_size: int = 64, epochs: int = 1_000, learning_rate: float = 1e-3, percent: float = 0.7):
        ...
        for ep in tqdm(range(1, epochs + 1), desc = 'Treinamento'):
            ...
            if ep % checkpoints == 0:
                ...
                self.neural_network.save(file_name + f'/model-{ep}.pt')
                with open(file_name + f'/trainning-{ep}.dat', 'wb') as arq: np.save(arq, loss_total)
