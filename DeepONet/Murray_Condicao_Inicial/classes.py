...
class Database:
    ...
    def generate_data(self):
        ...
        branch = np.zeros((self.data_size, 100), dtype = np.float32)
        initial_conditions = self.generate_initial_conditions()
        for i in range(self.data_size): branch[i] = initial_conditions[i][::5]
        self.branch = branch
        ...
        for i in tqdm(range(self.data_size), desc = 'Gerando dados de treinamento'):
            ...
            model = Rockne_FDM(20, self.time_size, 0.0039, 0.0453, 0.0305, 10., self.lenght_size)
